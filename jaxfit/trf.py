...
from jax.numpy.linalg import qr as jax_qr
from jax import jit
...
from jaxfit.lsmr_jit import lsmr
...
class TrustRegionJITFunctions():
    ...
    @jit
    def create_right_numtiplied_operator(self):
        def right_multiplied_operator(J: jnp.ndarray, d: jnp.ndarray):
            """Return J diag(d) as a function that can be used in JAX operations."""
    
            def matvec(x):
                return jnp.dot(J, x * d)  # Element-wise multiplication and matrix-vector multiplication
    
            def matmat(X):
                return jnp.dot(J, X * d[:, jnp.newaxis])  # Scaling each column of X by d
    
            def rmatvec(x):
                return d * jnp.dot(J.T, x)  # Element-wise multiplication after matrix-vector multiplication
        
        self.right_multiplied_operator = right_multiplied_operator
...
class TrustRegionReflective(TrustRegionJITFunctions):
    ...
    def trf_no_bounds(self, 
        ...
        while True:
            ...
            if tr_options['method'] == 'exact':
                svd_output = self.svd_no_bounds(J, d_jnp, f)
                J_h = svd_output[0]
                s, V, uf = [np.array(val) for val in svd_output[2:]]
            elif tr_options['method'] == 'lsmr':
                J_h = J * d
                if regularize:
                    a, b = self.cJIT.build_quadratic_1d(J_h, g_h_jnp, -g_h_jnp)
                    to_tr = Delta / norm(g_h_jnp)
                    ag_value = minimize_quadratic_1d(a, b,0, to_tr)[1]
                    reg_term = -ag_value / Delta**2

                damp_full = (damp**2 + reg_term)**0.5
                gn_h = lsmr(J_h, f, damp=damp_full, atol=1e-9, show=True, maxiter=5000)[0]
                S = jnp.vstack([g_h_jnp, gn_h]).T
                S, _ = jax_qr(S, mode='reduced')
                JS = jax.jit(jnp.dot)(J_h, S)
                B_S = jax.jit(jnp.dot)(JS.T, JS)
                g_S = jax.jit(jnp.dot)(S.T, g_h_jnp)
            ...
            while actual_reduction <= 0 and nfev < max_nfev:
                if tr_options['method'] == 'exact':
                    ...
                    p_S, _ = self.cJIT.solve_trust_region_2d(B_S, g_S, Delta)
                    step_h = jnp.dot(S, p_S)
